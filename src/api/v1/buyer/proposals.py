...
from src.modules.proposals.schemas import ProposalAcceptRequest, ProposalMessageCreate
...
async def list_proposal_messages(
    ...
    return await ProposalService(db).list_messages(proposal_id, ctx.actor.id, as_buyer=True)


@router.post("/proposals/{proposal_id}/messages")
async def send_proposal_message(
    ...
    return await ProposalService(db).add_message(
        proposal_id, ctx.actor.id, ctx.user.id, data, as_buyer=True
    )
