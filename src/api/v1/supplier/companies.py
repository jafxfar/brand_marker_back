...
router = APIRouter(prefix="/companies", tags=["supplier-companies"])
...
async def create_company(
    ...
    await SubscriptionService(db).check_company_limit(ctx.user)
    return await CompanyService(db).create_from_wizard(
        ctx.user, data, ActorType.supplier
    )
